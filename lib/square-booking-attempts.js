...
import { MANUAL_SQUARE_BOOKING_SOURCE } from "./tracks.js";
...
export async function syncSquareBookingAttempt(
  supabase,
  booking,
  { client, catalogById, now = new Date() } = {},
) {
  ...
  const { data: existing, error: loadError } = await supabase
    ...
    .select("id, source, revenue_status, phone")
  ...
  const insert = {
    ...
    booked_revenue_cents: catalogPriceCents,
