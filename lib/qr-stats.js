...
/**
 * Maintenance details get a maintenance QR card; all other completed details get general.
 */
export function classifyDetailForQrCard(serviceType) {
  const label = (serviceType ?? "").toLowerCase();
  if (!label.trim()) return null;
  if (label.includes("maintenance") || label.includes("entretien")) {
    return "qr_maintenance";
  }
  return "qr_general";
}
...
async function fetchCompletedDetailsSince(supabase, startDateYmd) {
  ...
  while (true) {
    const { data, error } = await supabase
      ...
      .select("id, completed_at, service_type")
...
function countCardsHandedOut(details) {
  ...
  for (const row of details) {
    const cardType = classifyDetailForQrCard(row.service_type);
