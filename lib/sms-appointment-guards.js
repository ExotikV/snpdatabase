...
export async function loadSmsAppointmentGuards(supabase, now = new Date()) {
  const [upcomingClientIds, cancellationCooldownClientIds] = await Promise.all([
  ...
  return {
    ...
    blockedClientIds: new Set([...upcomingClientIds, ...cancellationCooldownClientIds]),
  };
}
