...
async function loadSmsRefTriggerMap(supabase) {
  const map = new Map();
  let from = 0;
  ...
  while (true) {
    const { data, error } = await supabase
      .from("sms_log")
      .select("id, short_ref, trigger_type")
    ...
    for (const row of data) {
      map.set(row.id, row.trigger_type);
      if (row.short_ref) {
        map.set(row.short_ref, row.trigger_type);
      }
    }
...
function isManualSmsRef(ref, refTriggerMap) {
  return Boolean(ref && refTriggerMap.get(ref) === MANUAL_SMS_TRIGGER_TYPE);
}
...
function enrichSmsByTrack(sms, attempts, refTriggerMap) {
  const manualAttempts = attempts.filter((row) => isManualSmsRef(row.ref, refTriggerMap));

  return sms.byTrack.map((track) => {
    if (track.triggerType === MANUAL_SMS_TRIGGER_TYPE) {
      const bookings = manualAttempts.length;
      return {
        ...track,
        bookings,
        conversionRate: calcConversionRate(bookings, track.sent),
        bookedCents: sumRevenueCents(manualAttempts, "booked_revenue_cents"),
        actualCents: sumRevenueCents(manualAttempts, "actual_revenue_cents", {
          realizedOnly: true,
        }),
      };
    }
    ...
    const sourceRows = attempts.filter((row) => {
      if (normalizeBookingSource(row.source) !== source) return false;
      return !isManualSmsRef(row.ref, refTriggerMap);
    });
    const bookings = sourceRows.length;
    ...
    return {
      ...
      actualCents: sumRevenueCents(sourceRows, "actual_revenue_cents", {
        realizedOnly: true,
      }),
    };
  });
...
export async function getDashboardStats(supabase) {
  const [{ rows: attempts, includeRevenue }, sms, smsSubscribers, refTriggerMap] =
    await Promise.all([
      fetchAllBookingAttempts(supabase),
      getSmsDashboardStats(supabase),
      getSmsSubscriberStats(supabase),
      loadSmsRefTriggerMap(supabase),
    ]);
  ...
  const smsByTrack = enrichSmsByTrack(sms, attempts, refTriggerMap);
