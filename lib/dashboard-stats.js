...
function buildBookingCountsBySource(attempts) {
  const counts = Object.fromEntries(
    [...Object.keys(BOOKING_SOURCE_TO_TRIGGER), "qr_code"].map((source) => [source, 0]),
  );
...
function buildQrPerformanceRow(bookingCounts, attempts, includeRevenue) {
  const qrRows = attempts.filter((row) => normalizeBookingSource(row.source) === "qr_code");

  return {
    triggerType: "qr_code",
    label: BOOKING_SOURCE_LABELS.qr_code,
    sent: 0,
    failed: 0,
    converted: 0,
    bookings: bookingCounts.qr_code ?? 0,
    conversionRate: null,
...
export async function getDashboardStats(supabase) {
  ...
  const qrPerformance = buildQrPerformanceRow(bookingCounts, attempts, includeRevenue);
