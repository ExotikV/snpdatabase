...
import { assertSequenceCanBeRetried } from "./sms-retry.js";
import { appendOptOutFooter, assertClientCanReceiveSms } from "./sms-opt-out.js";
import {
  clientHasCompletedAppointment,
  NO_COMPLETED_APPOINTMENT_REASON,
...
export async function sendReminderToClient(supabase, client) {
  ...
  if (!(await clientHasCompletedAppointment(supabase, client.clientId))) {
    return {
      ok: false,
      clientId: client.clientId,
      name: client.name,
      track: client.track,
      reason: NO_COMPLETED_APPOINTMENT_REASON,
      noCompletedDetail: true,
    };
  }

  if (client.delayUnit !== "hours") {
  ...
  try {
    const failureGate = await assertSequenceCanBeRetried(supabase, {
      clientId: client.clientId,
      triggerType,
      sequenceNumber: client.sequenceNumber,
      cycleAnchor: client.cycleAnchor ?? client.lastDetailDate,
    });
    if (!failureGate.ok) {
      return {
        ok: false,
