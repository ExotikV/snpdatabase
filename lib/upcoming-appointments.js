...
async function loadClientsById(supabase) {
  ...
  while (true) {
    const { data, error } = await supabase
      .from("clients")
      .select("id, square_customer_id, name, phone, email, city, address")
...
async function getUpcomingFromDatabase(supabase, syncStats) {
  ...
  return {
    ...
    syncedAt: syncStats?.syncedAt ?? latestSyncedAt ?? now.toISOString(),
...
async function getUpcomingFromSquareLive() {
  ...
  const [clientsResult, attemptsBySquareId] = await Promise.all([
    supabase.from("clients").select("id, square_customer_id, name, phone, email, city, address"),
    loadBookingAttemptsBySquareId(supabase),
  ]);
 ...
 */
export async function getUpcomingAppointments({ syncMode = null } = {}) {
  const supabase = getSupabase();
  let syncStats = null;
