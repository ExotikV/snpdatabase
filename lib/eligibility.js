...
import { getClientsInSmsCooldown } from "./sms-cooldown.js";
import { getTriggerTypeForTrack, isGeneralTrack, MANUAL_SMS_TRIGGER_TYPE, TRACKS } from "./tracks.js";
...
function getRequiredDaysSince(context, nextStep, nextSequenceNumber) {
  if (!isGeneralTrack(context.track) || nextSequenceNumber !== 1) {
    return nextStep.days_since_last_detail;
  }

  const firstStepMinDays = getGeneralFirstReminderMinDays({ track: context.track });
...
export async function getEligibleClients(supabase, { clientId, track: trackFilter } = {}) {
  ...
  for (const client of clients) {
    ...
    if (smsCooldownByClient.has(client.id)) continue;
