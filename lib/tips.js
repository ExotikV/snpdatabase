...
/** Week runs Sunday through Saturday (Eastern). */
function getSundayOfWeek(parts) {
  const anchor = torontoCalendarToInstant(parts, 12);
  ...
  const offsets = { Sun: 0, Mon: 1, Tue: 2, Wed: 3, Thu: 4, Fri: 5, Sat: 6 };
...
export function resolveTipPeriodBounds(period, now = new Date(), { year: yearOverride } = {}) {
  ...
  if (period === "this_week") {
    ...
    const nextSunday = addTorontoDays(sunday, 7);
    const range = dayRangeFromParts(sunday, nextSunday);
    return { period, label: "This week", ...range };
  }

  if (period === "last_week") {
    const thisSunday = getSundayOfWeek(today);
    const lastSunday = addTorontoDays(thisSunday, -7);
    const range = dayRangeFromParts(lastSunday, thisSunday);
  ...
  if (monthMatch) {
    ...
    if (month >= 1 && month <= 12) {
      const resolvedYear = yearOverride ?? today.year;
...
export async function deleteTip(supabase, tipId) {
  ...
  if (!probe.ready) {
    if (probe.missing !== false) {
      throw new Error("Run schema/tips.sql in Supabase SQL Editor before deleting tips.");
    }
    throw new Error(probe.message ?? "Tips table is not available yet.");
  }

  const { data, error } = await supabase.from("tips").delete().eq("id", id).select("id").maybeSingle();
