...
/** Elapsed minutes since an instant (for post-detail review SMS). */
export function minutesSinceInstant(earlier, later = new Date()) {
  const from = earlier instanceof Date ? earlier : new Date(String(earlier));
  const to = later instanceof Date ? later : new Date(String(later));
  if (Number.isNaN(from.getTime()) || Number.isNaN(to.getTime())) return null;
  return Math.max(0, (to.getTime() - from.getTime()) / (60 * 1000));
}

/** UTC instant for 00:00 on a Toronto calendar day (inclusive day boundary). */
export function torontoDayStartIso(ymd) {
  const parts = getTorontoDateParts(ymd);
  if (!parts) return null;
  return torontoCalendarToInstant(parts, 0, 0).toISOString();
}

/** True when an instant falls on startYmd or a later Toronto calendar day. */
export function isOnOrAfterTorontoDate(isoTimestamp, startYmd) {
  if (!isoTimestamp || !startYmd) return false;
  const dateYmd = toDateInputValue(isoTimestamp);
  if (!dateYmd) return false;
  return dateYmd >= startYmd;
}
