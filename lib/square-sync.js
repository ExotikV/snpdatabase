...
function formatSquareAddress(address) {
  ...
  const parts = [line1, line2, cityRegion, postal].filter(Boolean);
  return parts.length > 0 ? parts.join(", ") : null;
}

export function getCustomerAddress(customer) {
  const fromPrimary = formatSquareAddress(customer.address);
  if (fromPrimary) return fromPrimary;

  for (const address of customer.addresses ?? []) {
    const formatted = formatSquareAddress(address);
