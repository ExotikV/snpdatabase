...
export function isClientOptedOut(client) {
  return Boolean(client?.opted_out);
}

export async function getClientOptOutStatus(supabase, clientId) {
  const { data, error } = await supabase
    .from("clients")
    .select("id, name, opted_out, opted_out_source, opted_out_at")
    .eq("id", clientId)
    .maybeSingle();
...
/**
 * Last-line check before any outbound SMS. Returns { ok: false } if opted out or missing.
 */
export async function assertClientCanReceiveSms(supabase, clientId) {
  const client = await getClientOptOutStatus(supabase, clientId);
  if (!client) {
    return { ok: false, reason: "Client not found" };
  }
  if (isClientOptedOut(client)) {
    return {
      ok: false,
      optedOut: true,
      reason: `${getOptOutStatusLabel(client.opted_out_source)} — SMS will not be sent`,
    };
  }
