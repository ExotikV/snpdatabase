...
import {
  ...
  SMS_COOLDOWN_DAYS,
  getClientsInSmsCooldown,
} from "./sms-cooldown.js";
...
function formatSendTiming({ status, daysUntilSend, inSendWindow }) {
  ...
  if (status === "blocked_cooldown") {
    return "Waiting for SMS cooldown";
  }
...
function mapRow(context, extra) {
  return {
    ...
    lastSmsSentAt: extra.lastSmsSentAt ?? null,
  };
}
...
export async function getSmsSchedulePreview(supabase) {
  ...
  const smsCooldownByClient = await getClientsInSmsCooldown(supabase, clientIds);
  ...
  for (const client of clients) {
    ...
    const lastSmsSentAt = smsCooldownByClient.get(client.id) ?? null;
    const onCooldown = Boolean(lastSmsSentAt);
    ...
    if (onCooldown) {
      blocked.push(
        mapRow(context, {
          ...
          inSendWindow,
          blockReason: `Texted within the last ${SMS_COOLDOWN_DAYS} days`,
          lastSmsSentAt,
        }),
      );
      continue;
  ...
  return {
    ...
    rules: {
      ...
      cooldownDays: SMS_COOLDOWN_DAYS,
      maxPerHour: MAX_SCHEDULED_SMS_PER_RUN,
      note: "Automated sequences only. Bulk manual SMS is separate and not listed here.",
    },
    summary: {
      ...
      blockedCooldown: blocked.length,
