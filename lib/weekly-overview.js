...
import { loadSquareRevenueForPeriod } from "./square-revenue-dashboard.js";
...
export async function getWeeklyOverview(supabase, { syncFirst = false } = {}) {
  ...
  const squarePeriod = await loadSquareRevenueForPeriod(bounds, now);
  const actualRevenueCents = squarePeriod.unavailable
    ? appointmentStats.actualRevenueCents
    : squarePeriod.totalCents;
  const squareOrderCount = squarePeriod.orderCount;
